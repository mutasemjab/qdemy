...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Montserrat+Arabic:wght@400;500;600;700&display=swap" rel="stylesheet">
...
document.querySelectorAll('.cmty-more').forEach(btn=>{
  btn.addEventListener('click',()=>{
    ...
    box.style.display = box.style.display==='none' ? 'flex' : (box.style.display ? 'none' : 'none');
    if(!btn.dataset.state){btn.dataset.state='open'}
    btn.dataset.state = btn.dataset.state==='open' ? 'closed' : 'open';
    btn.textContent = btn.dataset.state==='open' ? 'شاهد جميع التعليقات ←' : 'إخفاء التعليقات ←';
  });
});
</script>
...
document.querySelectorAll('.ud-send').forEach(b=>{
  b.addEventListener('click',()=>{
    ...
    chat.appendChild(m); box.value=''; chat.scrollTop = chat.scrollHeight;
  });
});
</script>
...
(() => {
  ...
  let index = 0, perView = 3, pages = 1, startX = 0, curX = 0, dragging = false;
  ...
  viewport.addEventListener('pointerdown', e => { dragging = true; startX = curX = e.clientX; viewport.setPointerCapture(e.pointerId); });
...
(function(){
  ...
  win.addEventListener('pointerdown', e=>{ startX = e.clientX; moved=0; win.setPointerCapture(e.pointerId); });
  win.addEventListener('pointermove', e=>{ if(startX!==null) moved = e.clientX - startX; });
  win.addEventListener('pointerup',   ()=>{
    if(Math.abs(moved)>40){ (moved>0?prev:next).click(); }
    startX=null; moved=0;
  });
...
<?php /**PATH J:\xampp-8.1.1\htdocs\qdemy-main\resources\views/layouts/app.blade.php ENDPATH**/ ?>