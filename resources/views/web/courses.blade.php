            ...
            <div class="course-card__body">
                <div class="course-card__crumb">
                    <span>
                        ...
                        @if($course->teacher) - {{ $course->teacher->name }} @endif
                        @if($course->grade) - {{ $course->grade->localized_name }} @endif
                    </span>
                </div>

                <a class="course-card__title" href="{{route('course',['course'=>$course->id,'slug'=>$course->slug])}}">
                ...
                <div class="course-card__cta">
                    @if(is_array($user_enrollment_courses) && in_array($course->id,$user_enrollment_courses))
                        <a href="{{route('course',['course'=>$course->id,'slug'=>$course->slug])}}" class="btn btn--outline">{{ translate_lang('enter_course') }}</a>
                    ...
                    @elseif(is_array($user_courses) && in_array($course->id,$user_courses))
                        <a href="{{ route('checkout') }}" class="btn btn--outline">{{translate_lang('go_to_checkout')}}</a>
                        <a href="{{route('course',['course'=>$course->id,'slug'=>$course->slug])}}" class="btn btn--solid">{{ translate_lang('enter_course') }}</a>
                    @else
                        <a href="{{route('course',['course'=>$course->id,'slug'=>$course->slug])}}" class="btn btn--outline">{{ translate_lang('enter_course') }}</a>
                        ...
                        <a href="{{ route('checkout') }}" id="go_to_checkout_{{$course->id}}" style="display:none;" class="btn btn--solid">
...
class EnrollmentManager {
    ...
    setupEnrollmentButtons() {
        ...
        enrollButtons.forEach(button => {
            button.addEventListener('click', async (e) => {
                e.preventDefault();
                if (!this.user) {
                    this.showModal("{{ translate_lang('login_required') }}","{{ translate_lang('please_login_first') }}",false,'fa-exclamation-circle');
                ...
                try {
                    ...
                    if (data.success) {
                        this.showModal("{{ translate_lang('course_added') }}","{{ translate_lang('course_added_successfully') }}",true,'fa-check');
                        button.remove();
                        document.getElementById('go_to_checkout_'+courseId).style.display = 'block';
                    } else {
                        this.showModal("{{ translate_lang('error') }}",data.message || "{{ translate_lang('something_went_wrong') }}",false,'fa-exclamation-triangle');
                        button.innerHTML = originalText;
                        button.disabled = false;
                    }
                } catch (error) {
                    this.showModal("{{ translate_lang('error') }}","{{ translate_lang('connection_error') }}",false,'fa-exclamation-triangle');
                    button.innerHTML = originalText;
                    button.disabled = false;
                }
            });
    ...
    setupModalButtons() {
        ...
        checkoutBtn?.addEventListener('click', () => { window.location.href = '{{ route("checkout") }}'; });
        window.addEventListener('click', (event) => { if (event.target === this.modal) this.hideModal(); });
    }
    setupFilterForm() {
        ...
        [programmSelect, gradeSelect, subjectSelect].forEach(element => {
            element?.addEventListener('change', () => {
                if (element === programmSelect) { gradeSelect.value = ''; subjectSelect.value = ''; }
                else if (element === gradeSelect) { subjectSelect.value = ''; }
                form.submit();
            });
        });
