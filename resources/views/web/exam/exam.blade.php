        ...
        <div class="cmty-feed">
                ...
                <article class="cmty-post cmty-post--outlined">
                    ...
                    @if (!$result && !$current_attempts->count() && $can_add_attempt)
                        <div class="cmty-actions">
                            <form
                                action="{{ route('exam.start', ['exam' => $exam->id, 'slug' => $exam->slug]) }}"
                                method='post'>
                                @csrf
            ...
            @if ($result && !$current_attempt && $question_nm == 1)
                <article class="cmty-post cmty-post--outlined">
                    ...
                    @if ($exam->show_results_immediately)
                        <div class="cmty-actions">
                            <a href="{{ route($apiRoutePrefix . 'review.attempt', ['exam' => $exam->id, 'attempt' => $result->id]) }}"
            ...
            @if ($current_attempt && $question)
                <article class="cmty-post cmty-post--outlined">
                    ...
                    <form
                        action="{{ route($apiRoutePrefix . 'answer.question', ['exam' => $exam->id, 'question' => $question->id, 'page' => $question_nm]) }}"
                        method='post'>
                        @if (!$isApi)
                            @csrf
                        @endif
                    ...
                    @if ($current_attempt->answers->count() > 0)
                        <div style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #eee;">
                            <form
                                action="{{ route($apiRoutePrefix . 'finish.exam', ['exam' => $exam->id]) }}"
                                method='post'
                        ...
                        @foreach ($attempts as $attempt)
                            <div style="padding: 10px; margin: 10px 0; border: 1px solid #ddd; border-radius: 5px;">
                                <div>
                                    <strong>
                                        <a
                                            href="{{ route($apiRoutePrefix . 'review.attempt', ['exam' => $exam->id, 'attempt' => $attempt->id]) }}">
