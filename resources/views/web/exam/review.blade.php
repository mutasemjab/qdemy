        ...
        <div class="cmty-feed">
            ...
            <article class="cmty-post cmty-post--outlined">
                ...
                <div class="cmty-actions">
                    <a href="{{ route($apiRoutePrefix . 'exam', ['exam' => $exam->id, 'slug' => $exam->slug]) . $queryParams }}"
                        class="cmty-like">{{ translate_lang('العودة للامتحان') }}</a>
                </div>
            </article>
                    ...
                    <div class="cmty-text">
                        ...
                        @if ($question->explanation)
                            <div style="margin: 10px 0; padding: 10px; background: #f0f8ff; border-radius: 5px;">
                                <small><strong>{{ translate_lang('ملاحظة') }}:</strong>
                                ...
                                @if ($answer->selected_options)
                                    <ul style="margin: 10px 0;">
                                        @foreach ($answer->getSelectedOptionsModels() as $selected)
                                            <li
                                                style="color: {{ in_array($selected->id, $question->getCorrectOptions()->pluck('id')->toArray()) ? '#4CAF50' : '#f44336' }};">
                                                {{ $selected->option }}
                                    ...
                                    @foreach ($question->options as $option)
                                        @php
                                            $was_selected =
                                                ...
                                                in_array($option->id, $answer->selected_options);
            ...
            <article class="cmty-post cmty-post--outlined">
                ...
                <div
                    style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px; margin: 15px 0;">
                ...
                <div class="cmty-actions">
                    @if ($exam->result_attempts()?->count())
                        <a href="{{ route($apiRoutePrefix . 'exam', ['exam' => $exam->id, 'slug' => $exam->slug]) . $queryParams }}"
                            class="cmty-like">{{ translate_lang('عرض جميع المحاولات') }}</a>
                    @else
                        <a href="{{ route($apiRoutePrefix . 'exam', ['exam' => $exam->id, 'slug' => $exam->slug]) . $queryParams }}"
                            class="cmty-like">{{ translate_lang('العودة للامتحان') }}</a>
                    @endif
