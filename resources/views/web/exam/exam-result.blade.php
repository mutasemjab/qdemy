    ...
    <div class="result-container">
        ...
        <div class="result-summary">
            <div class="result-header">
                <h1>{{ $exam->title }}</h1>
                <p class="result-date">{{ __('front.attempt_date') }}: {{ $attempt->created_at->format('d/m/Y H:i') }}</p>
            </div>
                ...
                <div class="question-review {{ $isCorrect ? 'correct' : 'incorrect' }}">
                    ...
                    @if($question->photo)
                        <div class="question-image" style="margin: 16px 0;">
                            <img src="{{ asset('assets/admin/uploads/' . $question->photo) }}" alt="{{ __('front.question_image') }}" style="max-width: 100%; max-height: 300px; border-radius: 6px;" />
                        </div>
                    @endif
                    ...
                    @if($question->type === 'multiple_choice')
                        <div class="question-options">
                            @foreach($question->options as $option)
                                <div class="option-item {{ in_array($option->id, (array)$answer->selected_options) ? 'selected' : '' }} {{ $option->is_correct ? 'correct-answer' : '' }}">
                                    <input type="checkbox" disabled {{ in_array($option->id, (array)$answer->selected_options) ? 'checked' : '' }}>
                    ...
                    @elseif($question->type === 'true_false')
                        @php
                            ...
                            $userSelectedTrue = isset($answer->selected_options[0]) && $answer->selected_options[0] === true;
                        @endphp
                        <div class="true-false-review">
                            <div class="answer-item {{ $userSelectedTrue ? 'selected' : '' }} {{ $correctAnswer ? 'correct-answer' : '' }}">
                                <i class="fas fa-check"></i> {{ __('front.صحيح') }}
                                @if($correctAnswer)
                                    <span class="correct-badge">✓</span>
                                @endif
                            </div>
                            <div class="answer-item {{ !$userSelectedTrue && $answer->selected_options ? 'selected' : '' }} {{ !$correctAnswer ? 'correct-answer' : '' }}">
                                <i class="fas fa-times"></i> {{ __('front.خطأ') }}
                                @if(!$correctAnswer)
                                    <span class="correct-badge">✓</span>
                                @endif
                    ...
                    @elseif($question->type === 'essay')
                        <div class="essay-review">
                            <div class="user-answer">
                                ...
                                <p>{{ !empty($answer->essay_answer) ? $answer->essay_answer : __('front.لم يتم الإجابة') }}</p>
                            </div>
                        ...
                        <div class="feedback error-feedback">
                            ...
                            <p>
                                ...
                                @elseif($question->type === 'multiple_choice')
                                    <br><small>الإجابات الصحيحة: {{ $question->options()->where('is_correct', true)->get()->pluck('option')->join(' و ') }}</small>
                                @elseif($question->type === 'true_false')
                                    <br><small>الإجابة الصحيحة: {{ $correctAnswer ? __('front.صحيح') : __('front.خطأ') }}</small>
                                @endif
                            </p>
                        </div>
                    @else
