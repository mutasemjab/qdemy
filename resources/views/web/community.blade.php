                    ...
                    <div class="cmty-post cmty-post--outlined">
                        <header class="cmty-head">
                            ...
                            <div class="cmty-user">
                                ...
                                <img src="{{ Auth::user()->photo_url ?? asset('assets_front/images/Profile-picture.jpg') }}"
                                    alt="">
                            </div>
                        </header>
                ...
                <article class="cmty-post {{ $loop->first ? 'cmty-post--outlined' : '' }}">
                    <header class="cmty-head">
                        ...
                        <div class="cmty-user">
                            ...
                            <img src="{{ $post->user->photo_url ?? asset('assets_front/images/Profile-picture.jpg') }}"
                                alt="">
                        </div>
                    </header>
                    ...
                    <div class="cmty-actions">
                        @auth
                            <!-- Comment Form -->
                            <form class="cmty-comment-form" action="{{ route('community.comments.store', $post) }}"
                                method="POST">
                                @csrf
                                <div class="comment-input-container">
                                    <input class="cmty-input" name="content" type="text"
                                        placeholder="{{ __('front.add_comment') }}" required maxlength="500">
                        ...
                        <!-- Like Button -->
                        <button class="cmty-like {{ Auth::check() && $post->isLikedBy(Auth::id()) ? 'liked' : '' }}"
                            data-post-id="{{ $post->id }}" {{ !Auth::check() ? 'disabled' : '' }}>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.querySelectorAll('.cmty-like').forEach(button => {
                    if (!button.disabled) {
                        button.addEventListener('click', function() {
                            ...
                            fetch(`{{ route('community.posts.toggle-like', ':postId') }}`.replace(
                                    ':postId', postId), {
                                    method: 'POST',
                                    headers: {
                ...
                document.querySelectorAll('.cmty-more').forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        const currentComments = this.closest('.cmty-post').querySelector(
                            '.cmty-comments');

                        if (commentsContainer.style.display === 'none') {
                            // Load all comments
                            fetch(`{{ route('community.posts.comments', ':postId') }}`.replace(
                                    ':postId', postId))
                                .then(response => response.json())
                                .then(data => {
