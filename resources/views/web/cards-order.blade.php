...
@section('content')
    <section class="co-page">
        ...
        <div class="examx-filters" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="200">
            <form action='{{ route('card-order') }}' method='get' id="filterForm">
                <div class="examx-row">
                    <div class="examx-dropdown">
                        <select class="examx-pill" name="programm_id" id="programm_id">
                            <option value="">{{ __('front.select_program') }}</option>
                    ...
                    <div class="examx-dropdown" id="gradeSection" style="">
                        <select class="examx-pill" name="grade_id" id="grade_id">
                            <option value="">{{ __('front.select_grade') }}</option>
        ...
        <div class="co-grid" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="320">
            @foreach ($cards as $card)
                <a class="co-card" data-card-id="{{ $card->id }}" data-card-name="{{ $card->name }}"
                    data-card-price="{{ $card->price }}" data-category-id="{{ $card->category_id }}"
                    data-doseyats='@json(
                        $card->doseyats->map(function ($d) {
                            return ['id' => $d->id, 'name' => $d->name, 'photo' => $d->photo];
                        }))'>
        ...
        <div class="co-panels">
            <div id="panelWhatsapp" class="co-panel" style="display:none; padding: 20px" data-aos="zoom-in"
            ...
            <div id="panelCash" class="co-panel" style="display:none; padding: 20px" data-aos="zoom-in"
                data-aos-duration="1000">
            ...
            <div id="panelVisa" class="co-panel" style="display:none; padding: 20px" data-aos="zoom-in"
                data-aos-duration="1000">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Show/hide grade section based on program selection
            [programmSelect, gradeSelect].forEach(element => {
                if (element) {
            ...
            // Check if grade section should be visible
            if (gradeSelect && document.querySelectorAll('#grade_id option').length > 1) {
                gradeSection.style.display = 'flex';
            }
        });

        // Cards Order Logic
        document.addEventListener('DOMContentLoaded', function() {
            // WhatsApp numbers map from server
            const whatsappNumbersMap = {!! json_encode($whatsappNumbers) !!};
            const categoriesMap = {!! json_encode($categories->mapWithKeys(function($cat) { return [$cat->id => $cat->localized_name]; })->toArray()) !!};
            const defaultWhatsappNumber = '+962775743580'; // Fallback

            let selectedAction = '';
            let selectedCategoryId = '';
            let selectedCategoryLabel = '';
            let selectedCard = null;
            let currentWhatsappNumber = defaultWhatsappNumber;
        ...
        `).join('');
            ...
            function updateSummaries() {
                ...
                updateDoseyats(doseyats, [{
                        container: 'waDoseyats',
                        list: 'waDoseyatsList'
                    },
                    {
                        container: 'cashDoseyats',
                        list: 'cashDoseyatsList'
                    },
                    {
                        container: 'visaDoseyats',
                        list: 'visaDoseyatsList'
                    }
                ]);

                if (payAmount) {
                    payAmount.textContent = selectedCard?.price ? (selectedCard.price + ' ' +
                        '{{ __('front.currency') }}') : '';
                }
            }
            ...
            cards.forEach(card => {
                card.addEventListener('click', function(e) {
                    ...
                    selectedCategoryLabel = categoriesMap[selectedCategoryId] || '{{ __('front.all_categories') }}';

                    // Update WhatsApp number based on selected category
                    if (selectedCategoryId && whatsappNumbersMap[selectedCategoryId]) {
                        currentWhatsappNumber = whatsappNumbersMap[selectedCategoryId];
                    } else {
                        currentWhatsappNumber = defaultWhatsappNumber;
                    }
            ...
            if (whatsappBtn) {
                whatsappBtn.addEventListener('click', function(e) {
                    ...
                    const url =
                        `https://wa.me/${currentWhatsappNumber.replace('+', '')}?text=${encodeURIComponent(msg)}`;
