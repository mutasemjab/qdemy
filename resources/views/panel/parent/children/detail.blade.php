...
@section('content')
    <section class="ud-wrap">
        ...
        <aside class="ud-menu">
            <div class="ud-user">
                <img data-src="{{ $childUser->photo_url }}" alt="">
                <div>
            ...
            <button class="ud-item" data-target="courses"><i class="fas fa-book"></i><span>{{ __('panel.courses') }}</span><i
                    class="fas fa-angle-left"></i></button>
        ...
        <div class="ud-content">
            ...
            <div class="ud-panel show" id="overview">
                ...
                <div class="ud-student-info-card">
                    <div class="ud-profile-head">
                        <img data-src="{{ $childUser->photo_url }}" class="ud-ava" alt="">
                        <div class="ud-name">
                        ...
                        <div class="ud-best-subject">
                            <div class="ud-achievement-badge">
                                ...
                                <div>
                                    ...
                                    <p>{{ $overallStats['best_subject']['name'] }} -
                                        {{ number_format($overallStats['best_subject']['average_score'], 1) }}%</p>
                                </div>
                            </div>
                        </div>
                    @endif
                        ...
                        <div class="ud-activity-timeline">
                            @foreach ($overallStats['recent_activity'] as $activity)
                                <div class="ud-activity-item">
                                    <div
                                        class="ud-activity-icon {{ $activity['score'] >= 70 ? 'success' : ($activity['score'] >= 50 ? 'warning' : 'danger') }}">
                                        <i class="fas fa-clipboard-check"></i>
                                    </div>
                            ...
                            <div class="ud-course-card">
                                <div class="ud-course-header">
                                    ...
                                    <div class="ud-course-progress-circle">
                                        <svg viewBox="0 0 36 36" class="circular-chart">
                                            ...
                                            <path class="circle" stroke-dasharray="{{ $course['progress'] }}, 100"
                                                d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" />
                                            <text x="18" y="20.35"
                                                class="percentage">{{ number_format($course['progress'], 0) }}%</text>
                                        </svg>
                                    </div>
                                </div>
                                ...
                                <div class="ud-course-body">
                                    ...
                                    <div class="ud-course-enrolled">
                                        <small>{{ __('panel.enrolled_on') }}:
                                            {{ $course['enrolled_at']->format('M d, Y') }}</small>
                                    </div>
                                </div>
                            </div>
                        @endforeach
            ...
            <div class="ud-panel" id="progress">
                ...
                @if ($courses->count() > 0)
                    <div class="ud-progress-courses-grid">
                        @foreach ($courses as $course)
                            <div class="ud-progress-course-card">
                                <div class="ud-progress-course-image">
                                    <img data-src="{{ $course['photo_url'] }}" alt="{{ $course['title'] }}">
                                    <div class="ud-progress-course-overlay">
                                        <span
                                            class="ud-progress-percentage">{{ number_format($course['progress'], 1) }}%</span>
                                    </div>
                                </div>
                                <div class="ud-progress-course-body">
                                    <h4>{{ Str::limit($course['title'], 35) }}</h4>
                                    <p class="ud-progress-subject">{{ $course['subject_name'] }}</p>
                                    <div class="ud-progress-bar-horizontal">
                                        <div class="ud-progress-fill-horizontal"
                        ...
                        <div class="ud-metric-card">
                            ...
                            <div class="ud-metric-body">
                                @php
                                    ...
                                    $trend =
                                        $recentExams->count() > 1
                                            ? ($recentExams->first()['percentage'] > $recentExams->last()['percentage']
                                                ? 'up'
                                                : 'down')
                                            : 'stable';
                                @endphp
                                <span class="ud-trend {{ $trend }}">
                                    <i
                                        class="fas fa-arrow-{{ $trend == 'up' ? 'up' : ($trend == 'down' ? 'down' : 'right') }}"></i>
                                    {{ ucfirst($trend) }}
                                </span>
                            </div>
                        </div>
...
@push('styles')
    <style>
        ...
        .ud-stat-icon.bg-primary {
            background: #007bff;
        }

        .ud-stat-icon.bg-success {
            background: #28a745;
        }

        .ud-stat-icon.bg-warning {
            background: #ffc107;
        }

        .ud-stat-icon.bg-info {
            background: #17a2b8;
        }
        ...
        .ud-activity-icon.success {
            background: #28a745;
        }

        .ud-activity-icon.warning {
            background: #ffc107;
        }

        .ud-activity-icon.danger {
            background: #dc3545;
        }
        ...
        .ud-course-card:hover .ud-course-header img {
        ...
        .ud-summary-card.success {
            border-left-color: #28a745;
        }

        .ud-summary-card.danger {
            border-left-color: #dc3545;
        }

        .ud-summary-card.warning {
        ...
        /* New Progress Panel Styles - Course Cards with Individual Progress */
        .ud-progress-courses-grid {
            display: grid;
            grid-template-columns: repeat(auto-fill, minmax(280px, 1fr));
            gap: 20px;
        }

        .ud-progress-course-card {
            background: white;
            border-radius: 16px;
            overflow: hidden;
            box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1);
        ...
        .ud-progress-course-card:hover {
            transform: translateY(-5px);
            box-shadow: 0 8px 25px rgba(0, 0, 0, 0.15);
        }

        .ud-progress-course-image {
            position: relative;
            height: 160px;
            overflow: hidden;
        }

        .ud-progress-course-image img {
            width: 100%;
            height: 100%;
            object-fit: cover;
        }

        .ud-progress-course-overlay {
            position: absolute;
            top: 12px;
            right: 12px;
        ...
        .ud-progress-percentage {
            display: block;
            line-height: 1;
        }

        .ud-progress-course-body {
            padding: 20px;
        }

        .ud-progress-course-body h4 {
            margin: 0 0 8px 0;
            color: #2c3e50;
            font-size: 16px;
            font-weight: 700;
        ...
        .ud-progress-fill-horizontal {
            height: 100%;
            background: linear-gradient(90deg, #0b57d0, #4a90e2);
            border-radius: 4px;
            transition: width 0.6s ease;
        }
        ...
        .ud-subject-average span,
        .ud-subject-passed span {
            display: block;
            font-size: 18px;
            font-weight: bold;
            color: #007bff;
        ...
        .ud-subject-average small,
        .ud-subject-passed small {
            color: #6c757d;
            font-size: 12px;
        }
