...
.container-limited{max-width:1300px;margin:0 auto;padding:16px}
...
.head-bar h2{margin:0;font-size:24px;color:var(--ink);font-weight:900;letter-spacing:.2px}
.subtle{margin:4px 0 0 0;color:var(--muted);font-size:13px}
.head-actions{display:flex;gap:8px;flex-wrap:wrap}
.btnx{display:inline-flex;align-items:center;gap:8px;border-radius:12px;padding:10px 14px;font-weight:800;font-size:14px;text-decoration:none;cursor:pointer;border:1px solid var(--line);background:#fff;color:var(--ink);transition:box-shadow .16s,transform .16s,border-color .16s,opacity .16s}
.btnx:disabled{opacity:.6;pointer-events:none}
.btnx:hover{transform:translateY(-1px);box-shadow:0 10px 22px rgba(15,23,42,.08)}
...
.input-sm{border:1px solid var(--line);border-radius:10px;padding:8px 10px;font-size:13px;min-width:220px}
...
.tablex thead th{position:sticky;top:0;z-index:1;background:#f8fafc;border-bottom:1px solid var(--line);padding:12px 12px;font-size:12px;color:var(--muted);font-weight:800}
.tablex tbody td{border-bottom:1px solid var(--line);padding:12px;vertical-align:middle}
  ...
  <div class="container-limited">
    <div class="head-bar">
      ...
      <div class="head-actions">
        <button type="button" class="btnx btnx-primary js-show-later" data-delay="5000">
          <i class="fas fa-bell"></i>{{ __('panel.view') }} • 5s
        </button>
...
<div class="modal fade" id="timedModal" tabindex="-1">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">{{ __('panel.notification') }}</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body">
        <div class="row-inline">
          <i class="fas fa-bell" style="color:var(--primary)"></i>
          <div style="font-weight:800">{{ __('panel.ready') }}</div>
        </div>
        <div class="meta-muted" style="margin-top:8px">{{ __('panel.action_completed') }}</div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btnx btnx-secondary" data-bs-dismiss="modal">{{ __('panel.close') }}</button>
      </div>
    </div>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (searchInput && searchInput.form) {
    ...
    searchInput.addEventListener('input', function () {
      ...
      to = setTimeout(function () {
        if (searchInput.value.trim() === '' && !new URLSearchParams(window.location.search).has('search')) return;
  ...
  document.querySelectorAll('.js-answers').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      fetch(`{{ route('teacher.exams.index') }}/{{ $exam->id }}/attempts/${attemptId}/answers`, {headers:{'X-Requested-With':'XMLHttpRequest'}})
        .then(function(r){return r.ok?r.json():Promise.reject()})
        .then(function(d){content.innerHTML = d.html || '<div class="meta-muted">{{ __("panel.error_loading_answers") }}</div>'})
        .catch(function(){content.innerHTML = '<div class="meta-muted">{{ __("panel.error_loading_answers") }}</div>'});
    });
  });
  ...
  document.querySelectorAll('.js-download').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      link.href = `{{ route('teacher.exams.index') }}/{{ $exam->id }}/attempts/${attemptId}/download`;
      ...
      fetch(`{{ route('teacher.exams.index') }}/{{ $exam->id }}/attempts/${attemptId}/manual-grade`, {
        ...
        .then(function(d){
          if (d && d.success) {
            ...
            var err = document.getElementById('manualError');
            err.textContent = (d && d.message) ? d.message : '{{ __("panel.error_updating_grade") }}';
        ...
        .catch(function(){
          var err = document.getElementById('manualError');
          err.textContent = '{{ __("panel.error_updating_grade") }}';
          err.classList.remove('hidden');
          manualSubmit.disabled = false;
          manualSubmit.textContent = '{{ __("panel.update_grade") }}';
        });
  ...
  document.querySelectorAll('.js-show-later').forEach(function(btn){
    btn.addEventListener('click', function(){
      var delay = parseInt(this.dataset.delay || '3000',10);
      var original = this.innerHTML;
      this.disabled = true;
      this.innerHTML = '<i class="fas fa-hourglass-half"></i>';
      setTimeout(()=>{
        this.disabled = false;
        this.innerHTML = original;
        var tm = new bootstrap.Modal(document.getElementById('timedModal'));
        tm.show();
      }, delay);
    });
  });
  ...
  @if($stats['total_attempts'] > 0)
      ...
      new Chart(scoreCtx, {
        ...
        data: {
          ...
          datasets: [{
            ...
            backgroundColor: ['#ef4444','#f59e0b','#fb923c','var(--primary)','#10b981'],
            borderRadius: 6,
            maxBarThickness: 42
          }]
        },
    ...
    var pfCanvas = document.getElementById('passFailChart');
    if (pfCanvas) {
