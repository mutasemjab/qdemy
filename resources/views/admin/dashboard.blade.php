...
@section('css')
    <style>
        ...
        .stats-card .icon.exams {
            background: linear-gradient(45deg, #a8edea, #fed6e3);
        }

        .stats-card .icon.questions {
            background: linear-gradient(45deg, #ffecd2, #fcb69f);
        }

        .stats-card .icon.attempts {
            background: linear-gradient(45deg, #667eea, #764ba2);
        }

        .stats-card .icon.notifications {
        ...
        .progress-fill.students {
            background: linear-gradient(45deg, #4facfe, #00f2fe);
        }

        .progress-fill.teachers {
            background: linear-gradient(45deg, #43e97b, #38f9d7);
        }
        ...
        <div class="row mb-4">
            ...
            <div class="col-lg-3 col-md-6 mb-4">
                <div class="stats-card">
                    ...
                    <div class="progress-bar-custom">
                        <div class="progress-fill students"
                            style="width: {{ $totalStudents > 0 ? ($activeStudents / $totalStudents) * 100 : 0 }}%"></div>
                    </div>
                    <small class="text-muted mt-2 d-block">{{ $activeStudents }}/{{ $totalStudents }}
                        {{ __('messages.active') }}</small>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6 mb-4">
                <div class="stats-card">
                    ...
                    <div class="progress-bar-custom">
                        <div class="progress-fill teachers"
                            style="width: {{ $totalTeachers > 0 ? ($activeTeachers / $totalTeachers) * 100 : 0 }}%"></div>
                    </div>
                    <small class="text-muted mt-2 d-block">{{ $activeTeachers }}/{{ $totalTeachers }}
                        {{ __('messages.active') }}</small>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6 mb-4">
                <div class="stats-card">
                    ...
                    <div class="progress-bar-custom">
                        <div class="progress-fill exams"
                            style="width: {{ $totalCourses > 0 ? ($activeCourses / $totalCourses) * 100 : 0 }}%"></div>
                    </div>
                    <small class="text-muted mt-2 d-block">{{ $activeCourses }}/{{ $totalCourses }}
                        {{ __('messages.active') }}</small>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6 mb-4">
                <div class="stats-card">
                    ...
                    <div class="progress-bar-custom">
                        <div class="progress-fill exams"
                            style="width: {{ $totalExams > 0 ? ($activeExams / $totalExams) * 100 : 0 }}%"></div>
                    </div>
                    <small class="text-muted mt-2 d-block">{{ $activeExams }}/{{ $totalExams }}
                        {{ __('messages.active') }}</small>
                </div>
            </div>
        </div>
            ...
            <div class="col-lg-3 col-md-6 mb-4">
                <div class="stats-card">
                    ...
                    <small class="text-muted">
                        {{ __('messages.mc_symbol') }}: {{ $multipleChoiceQuestions }} | {{ __('messages.tf_symbol') }}:
                        {{ $trueFalseQuestions }} | {{ __('messages.essay_symbol') }}: {{ $essayQuestions }}
                    </small>
                </div>
            </div>
                        ...
                        @foreach($topStudents as $index => $student)
                            <div class="top-student-item">
                                <div class="d-flex align-items-center">
                                    <div
                                        class="student-rank {{ $index == 0 ? 'first' : ($index == 1 ? 'second' : ($index == 2 ? 'third' : 'other')) }}">
                                        {{ $index + 1 }}
                                    </div>
                                    <div>
                        ...
                        @foreach($recentExams as $exam)
                            <div class="activity-item">
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <p class="mb-1 text-muted">{{ $exam->course->title_en ?? __('messages.no_course') }}</p>
                                    <small class="text-muted">
                                ...
                                <div class="text-right">
                                    ...
                                    <small class="d-block text-muted">{{ $exam->questions->count() }}
                                        {{ __('messages.questions') }}</small>
                                </div>
                            </div>
                        @endforeach
                    @else
