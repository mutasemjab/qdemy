...
class NotificationParentController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if (empty($notifications)) {
                // Get parent notifications using parent_id field
                $notificationQuery = Notification::where('parent_id', $user->id);
                ...
                return $this->success_response('Notifications retrieved successfully', [
                    ...
                    'summary' => [
                        ...
                        'unread_count' => Notification::where('parent_id', $user->id)->whereNull('read_at')->count()
                    ]
                ]);
            }
    ...
    public function markAsRead(Request $request, $notificationId)
    {
        try {
            ...
            // Find notification and ensure it belongs to this parent
    ...
    public function markAllAsRead(Request $request)
    {
        try {
            ...
            $updatedCount = Notification::where('parent_id', $user->id)
                ->whereNull('read_at')
                ->update(['read_at' => now()]);
